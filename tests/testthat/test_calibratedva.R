...
test_that("M-shrinkage works with single cause labels", {
    ...
    calibratedva_out <- calibratedva(va_unlabeled = A_U,
                                     va_labeled = A_L, gold_standard = G_L, causes = as.character(1:C),
                                    method = "mshrink", ndraws = 500, print.chains = FALSE, burnin = 1, thin = 1)
...
test_that("M-shrinkage works with multi cause labels", {
    ...
    calibratedva_out <- calibratedva(va_unlabeled = A_U, va_labeled = A_L, gold_standard = G_L, causes = as.character(1:C),
                                     method = "mshrink", ndraws = 500, print.chains = FALSE, burnin = 1, thin = 1)
    samples <- calibratedva_out$samples
    p_samples <- do.call(rbind, lapply(1:3, function(chain) {
        samples[[chain]][,1:3]
...
test_that("M-shrinkage works with ensemble multi cause labels", {
    ...
    calibratedva_out <- calibratedva(va_unlabeled = A_U, va_labeled = A_L, gold_standard = G_L,
...
test_that("M-shrinkage gives good estimate of p with ensemble multi cause labels", {
    ...
    plan(multisession)
    calibratedva_out <- calibratedva(va_unlabeled = A_U, va_labeled = A_L, gold_standard = G_L, causes = as.character(1:C),
                                     method = "mshrink", ndraws = 5000, print.chains = FALSE, burnin = 1000, thin = 1)
    samples <- calibratedva_out$samples
    p_samples <- do.call(rbind, lapply(1:3, function(chain) {
        samples[[chain]][,1:3]
    }))
...
test_that("p-shrinkage works with single cause labels", {
    ...
    calibratedva_out <- calibratedva(va_unlabeled = A_U, va_labeled = A_L, gold_standard = G_L, causes = as.character(1:C),
                                     method = "pshrink", ndraws = 500, print.chains = FALSE, burnin = 1, thin = 1)
    samples <- calibratedva_out$samples
    p_samples <- do.call(rbind, lapply(1:3, function(chain) {
        samples[[chain]][,1:3]
    }))
...
test_that("p-shrinkage gives good estimate of p with ensemble multi cause labels", {
    ...
    plan(multisession)
    calibratedva_out <- calibratedva(va_unlabeled = A_U, va_labeled = A_L, gold_standard = G_L, causes = as.character(1:C),
                                     method = "pshrink", ndraws = 5000, print.chains = FALSE, burnin = 1000, thin = 1)
    samples <- calibratedva_out$samples
    p_samples <- do.call(rbind, lapply(1:3, function(chain) {
        samples[[chain]][,1:3]
    }))
...
test_that("M-shrinkage cross-validation works with ensemble multi cause labels", {
    ...
    calibratedva_out <- tune_calibratedva(va_unlabeled = A_U, va_labeled = A_L,
                                          gold_standard = G_L, causes = as.character(1:C),
                                          method = "mshrink", alpha_vec = alpha_vec,
                                          ndraws = 50, print.chains = FALSE, burnin = 1, thin = 1)
...
test_that("p-shrinkage cross-validation works when provided with a samples list", {
    set.seed(123)
    N_U <- 1000
    N_L <- 100
    C <- 3
    ...
    log10vec <- c(-3, -2, seq(-1, 2, length = 3))
    lambda_vec <- 10^log10vec
    samples_list <-lapply(lambda_vec[c(4,1,3,2,5)], function(lambda) {
        calibrateva_out <- calibratedva(va_unlabeled = A_U,
                                        va_labeled = A_L,
                                        gold_standard = G_L, causes = as.character(1:C),
                                        method = "pshrink", lambda = lambda,
                                        ndraws = 50, print.chains = FALSE, burnin = 1, thin = 1)
        return(calibrateva_out)
    })
    calibratedva_original <- tune_calibratedva(va_unlabeled = A_U, va_labeled = A_L,
                                          gold_standard = G_L, causes = as.character(1:C),
                                          method = "pshrink", lambda_vec = lambda_vec,
                                          ndraws = 50, print.chains = FALSE, burnin = 1, thin = 1)
    calibratedva_out <- tune_calibratedva(va_unlabeled = A_U, va_labeled = A_L,
                                          gold_standard = G_L, causes = as.character(1:C),
                                          method = "pshrink", samples_list = samples_list)
    expect_equal(names(calibratedva_out),
                 c("final_model", "alpha_final", "lambda_final", "waic_df"))
    samples <- calibratedva_out$final_model$samples
    expect_equal(class(samples), "mcmc.list")
    expect_equal(calibratedva_out$final_model$lambda, calibratedva_out$lambda_final)
    expect_equal(calibratedva_out$final_model$method, "pshrink")
    waic_df <- calibratedva_out$waic_df
    expect_equal(class(waic_df), "data.frame")
    expect_equal(nrow(waic_df), length(lambda_vec))
    expect_true(identical(calibratedva_original$waic_df, calibratedva_out$waic_df))
    expect_true(identical(calibratedva_original$final_model$lambda, calibratedva_out$final_model$lambda))
})