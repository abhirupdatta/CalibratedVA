...
csmf.revamp <- sapply(causes, function(c) mean(tariff.revamp.csmf$p[tariff.revamp.csmf$cause == c]))
...
```{r}
csmf.insilico <- sapply(causes, function(c) mean(test.cod == c))
csmf.revamp.insilico <- sapply(causes, function(c) mean(insilico.revamp.csmf$p[insilico.revamp.csmf$cause == c]))
...
```{r format_ensemble}
test.cod.mat <- matrix(c(pred.cod, pred.cod.insilico), ncol = 2)
calib.cod.mat <- matrix(c(calib.cod, calib.cod.insilico), ncol = 2)
```

And we now we can run the function using nearly identical arguments to that of the
`revamp.sampler` function.
...
ensemble.revamp <- revamp.ensemble.sampler(test.cod.mat = test.cod.mat,
                                           ...
                                           epsilon = epsilon, alpha=alpha, beta=beta,
                                           tau.vec=tau.vec, delta=delta,
                                           gamma.init=gamma.init, ndraws = 5000)
```
...
We will obtain individual predictions from all three ReVAMP methods using the `revampIndPredictions`
method. It takes in the posterior samples from either the `revamp.sampler` or `revamp.ensemble.sampler` function, 
