...
We will split the PHMRC data from India into a calibration and test set,
...
For simplicity, we will restrict ourselves to the top 4 COD (from India), and treat
...
```{r top_4_cod}
top.cod <- names(sort(table(india.data$Cause), decreasing = TRUE))
top4.cod <- top.cod[top.cod != "14"][1:4]
change.cause <- function(cause) {
    cause[!(cause %in% top4.cod)] <- 99
    return(cause)
}
train.data$Cause <- change.cause(train.data$Cause)
test.data$Cause <- change.cause(test.data$Cause)
calibration.data$Cause <- change.cause(calibration.data$Cause)
```
...
We can inspect the CSMF accuracies again
...
tariff.revamp.ind.cod <- revampIndPredictions(tariff.revamp, test.cod = test.cod.tariff,
                                       causes = causes, burnin = 20000, thin = 5)$topCOD
