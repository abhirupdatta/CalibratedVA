...
This function utilizes the `ggs` function from the `ggmcmc` package. We will demonstrate
how to obtain CalibratedVA output using the output from Tariff.
...
with true COD 17 and 9. We can compare the posterior mean of the M matrix,
which is shrunken towards the identity matrix, to the empirical estimate of M.

```{r tariff_m_matrix_empirical}
