...
We will first run CalibratedVA using the Tariff predictions, and then use
a burn-in of 10,000 samples and a thinning parameter of 10.
...
tariff.calibva <- window(tariff.calibva, start = 1e4, thin = 5)
```
...
insilico.calibva <- calibva.sampler(test.cod = insilico.train.cod.test,
                                  ...
                                  nchains = nchains,
                                  init.seeds = calibva.seeds)
insilico.calibva <- window(insilico.calibva, start = 1e4, thin = 5)
```
...
ensemble.calibva <- calibva.ensemble.lite.sampler(test.cod.mat = test.cod.mat,
                                                  ...
                                                  nchains = nchains,
                                                  init.seeds = calibva.seeds)
ensemble.calibva <- window(ensemble.calibva, start = 1e4, thin = 5)
```
